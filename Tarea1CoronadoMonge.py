    ...
    if type(operando_1) == int and type(operando_2) == int and type(operacion) == int:
        if ( -128 < operando_1 < 128 and -128 < operando_2 < 128):
            if operacion == 1:
                ...
                print ("Error 3 = OpOutOfRangeError: la operación ingresada no está dentro de las opciones")
        else:
            print ("Error 2 = RangeError: Entrada fuera del rango válido ")
    else:
        ...
        if len(operando_1) == len (operando_2):
                ...
                for i in range(0, len(operando_1) ):
                        if type(operando_1[i]) == int and type(operando_2[i]) == int and type(operacion) == int:
                            if ( -128 < (operando_1[i]) < 128 and -128 < (operando_2[i]) < 128):
                                if 1 <= operacion <= 4:
